...
const getAllComment = async (req, res) => {
  try {
    ...
    const post = await PostModel.findById(postId).populate("comments.user", "name email");
    if (!post) {
      return res
        .status(404)
        .json({ success: false, message: "Post not found" });
    ...
    res.status(200).json({ success: true, conut:sortedComments.length, comments: sortedComments });
  } catch (err) {
    console.error(err);
